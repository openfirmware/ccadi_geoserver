...
active_cert_path = selfsigned_certificate_path

# Use an attribute flag to only enable fetching HTTPS certificates in production.
# In testing, getting certificates from Let's Encrypt doesn't work as the test VM isn't internet-facing.
if node["certbot"]["enabled"]
  domains = node["ccadi_geoserver"]["domains"].join(",")

  bash "get certificate using certbot" do
    code "certbot certonly --nginx -n --domains #{domains} --agree-tos -m #{node["certbot"]["email"]}"
  end

  # TODO: find the path generated by certbot
  active_cert_path = ""
end
